...
def _set_params(conf_file: Union[str, None] = None) -> None:
    ...
    INERTIAL_SENSORS = np.array(conf["inertial_sensor"], dtype=str)
...
def _format_log(src_file: str, tgt_dir: str) -> None:
    ...
    if not path.exists(dir):
        makedirs(dir)
    tgt_file = path.join(dir, str(resampled_ts[0].date()) + "_" + path.basename(src_file)[:-4] + ".csv")
    ...
    if ENABLE_BLE:
        ...
        if not path.exists(dir):
            makedirs(dir)
        tgt_file = path.join(dir, str(ble[0][0].date()) + "_" + path.basename(src_file)[:-4] + ".csv")
        with open(tgt_file, "w") as f:
            writer = csv.writer(f)
